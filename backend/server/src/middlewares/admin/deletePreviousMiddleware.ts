...
export const deletePreviousMagazineFiles = async (req: Request, res: Response, next: NextFunction) => {
  try {
    console.log(`Request ${JSON.stringify(req.query)}`);
    const pagesPath = req.query.pagesPath;

    if (!pagesPath || typeof pagesPath !== "string") {
      return res.status(400).json({ error: "pagesPath is required and must be a string" });
    }

    const magazineDir = path.resolve(pagesPath);
